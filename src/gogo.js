...
import {
 ...
 import * as cheerio from "cheerio";
  ...
  const BaseURL = const BaseURL = ["https://gogoanimes.org.za", "https://gogoanime3.co" , "https://gogoanimes.com.in/","https://gogoanime.co.ba/"]; 
  const USER_AGENT =
  ...
  async function getSearch(name, page = 1) {
    ...
    for (const url of BaseURL) {
      const response = await fetch(`${url}/search.html?keyword=${name}&page=${page}`);
      ...
      $("ul.items li").each(function (i, elem) {
        ...
        if (anime.link) anime.link = url + anime.link;
  
        searchResultsTemp.push(anime);
      });
  
      searchResults.push(...searchResultsTemp);
    }
    return searchResults;
  }
  
  async function getAnime(id) {
    let response = await fetch(`${BaseURL[0]}/category/${id}`);
    let html = await response.text();
    let $ = cheerio.load(html);
    let animeData = {
    ...
    for (const url of BaseURL) {
      ...
      $("ul.items li").each(function (i, elem) {
        ...
        const anime = {
          ...
          link: url + $("div.img a").attr("href") || null,
  ...
  async function getPopularAnime(page = 1, max = 10) {
    const popularAnime = [];
    for (const url of BaseURL) {
      const response = await fetch(`${url}/popular.html?page=${page.toString()}`);
      let html = await response.text();
      let $ = cheerio.load(html);
  ...
  async function GogoDLScrapper(animeid, cookie) {
    try {
      cookie = atob(cookie);
      const response = await fetch(`${BaseURL[0]}/${animeid}`, {
        headers: {
          Cookie: `auth=${cookie}`,
        },
      });
